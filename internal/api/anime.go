...
var (
	...
	reSourceTag = regexp.MustCompile(`(?i)[🔥🌐]?\[(?:animefire|allanime|anidb|animedrive|9anime|otakudesu|samehadaku|nimegami|ylnime|moenime|astronime)\]\s*`)
	...
	reSpaceDashNoise = regexp.MustCompile(`(?i)\s+-\s+(?:` +
		...
		`parte\s*\d+|part\s*\d+|` +
		`otakudesu|samehadaku|nimegami|ylnime|moenime|astronime` +
		`).*$`)
